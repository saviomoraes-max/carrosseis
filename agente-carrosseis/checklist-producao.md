...
4. **Prova conecta e não desmente** — o print prova a TESE deste post (não credita
   outra causa) e o punch argumenta sem ecoar o texto do print. Reuso de print
   publicado é LIBERADO (Sávio, 25/ago) — `prints_usados.py` vira MAPA, não veto:
   rodar pra saber onde o print já apareceu e declarar o reuso no ORIGEM.txt; o
   MESMO print não repete dentro da semana. **Prova é OBRIGATÓRIA em post de
   aplicação e isca** (slide penúltimo, antes do CTA).
...
- [ ] **Abri e OLHEI cada PNG** (Read em todos, SEMPRE que a copy ou imagem mudar):
  ...
  - **Proof — a SELEÇÃO de prints é do agente, nunca do usuário (regra do Sávio, 15/jul):** pool curado nos `img/depoimentos-sugeridos/` das semanas + `DEPOIMENTOS-MAP.md` + os `img/dep*.png` já anonimizados das semanas publicadas (fonte preferida pra reuso: crop/blur já feitos). **REUSO LIBERADO (Sávio, 25/ago):** pXX já publicado PODE voltar ("não tem problema eles já terem sido utilizados") — segue proibido o MESMO print duas vezes na mesma semana e o detalhe-estrela da mesma clínica em posts adjacentes (caso "tráfego R$250" — cropar a linha resolve); tema do print casa com a TESE do post (ticket alto pra post de preço, consultas pra post de consulta). O punch só fecha DEPOIS dos prints escolhidos, com número literal deles.
...
- [ ] **AUDITORIA DE EXPORT POR CHECKSUM (PARE — 27/ago):** antes de avisar "pronto",
  rodar md5 de CADA `[AA] [SS] [ADxxx_N]` da raiz contra o `slides/slide_N.png`
  correspondente, em TODOS os posts do lote. Qualquer divergência = export defasado:
  refazer e rodar de novo. *Caso real 27/ago: troca de foto vetada (criança) foi
  renderizada e conferida no slides/, mas o export da raiz ficou velho; o Sávio
  recebeu o arquivo errado DUAS vezes. O export é O produto; slides/ é rascunho.*
...
| 33 | **Imagem com texto embutido usada como fundo/cartão** (27/ago — o slide 2 do demo D entrou com card de anúncio da Sallve que tinha headline própria; o Sávio pegou: "pode ficar confuso para o leitor") | Foto de post/card alheio com TEXTO GRÁFICO nunca vira fundo nem cartão: ou acha foto limpa, ou CROPA o texto fora, **ou APAGA o texto por IA** — `limpar_texto.py` (LaMa/inpainting local, offline, sem chave; interpretador `/tmp/ia-img/bin/python3`, venv 3.10). Testado 29/ago num card da Natura com 3 blocos de texto: apagou sem rastro. Fundo liso sai perfeito; padrão complexo borra (aí cropar é melhor). Rótulo de embalagem pequeno, parte do objeto, é aceitável. Vale pra capa, slide interno e cartão `image` |
| 35 | **Mockup de e-commerce no lugar de fotografia** (29/ago — o Raio-X da Principia saiu com crops de card de anúncio: produto renderizado em fundo chapado, rótulo ampliado e ilegível; o Sávio: "absurdo de feio". Piorou ao compor produto vertical em cartão paisagem = 80% de fundo vazio) | Foto de marca vem de FOTOGRAFIA EDITORIAL (luz natural, textura, cena real), nunca de mockup/card de e-commerce — o design D pede "documentary, available light" pelo próprio readme. Onde buscar: feed de marcas com direção de arte (ex.: @augustinusbader), não feed de varejo. Produto vertical não preenche cartão paisagem: usar macro/detalhe ou textura. E crop exclui QUALQUER logo de terceiro do quadro |
| 34 | **Verificação por AMOSTRAGEM deixou criança passar** (27/ago — o slide 2 do AD019 foi ao export com foto de piscina COM CRIANÇA: troquei o rótulo de dois arquivos baixados na cabeça e conferi os slides 1/3/5/9, nunca o 2; o Sávio pegou no preview e perguntou "você não está conseguindo ver o que você está criando?". A varredura completa que veio em seguida achou OUTRA: crianças na grade atrás do Tom Holland no AD017) | Verificação visual NÃO é amostragem: depois de todo render, TODO slide que contém imagem (capa, photo, cartão, proof) é aberto no pixel, um a um, ANTES do export — e o ORIGEM registra "varredura N/N". Arquivo baixado nunca é identificado de memória: confere-se OLHANDO no momento do uso. Afirmar conteúdo de slide pro usuário sem ter aberto o PNG = proibido. **E o incidente teve 2º ato (mesmo dia):** a foto foi trocada, o slides/ re-renderizado e conferido, mas os EXPORTS renomeados da raiz não foram refeitos — o arquivo que o usuário posta continuou com a criança. Re-render sem re-export = mentira de prontidão |
