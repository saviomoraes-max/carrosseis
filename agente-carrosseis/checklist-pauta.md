...
- [ ] **A MECÂNICA dominante do TOP maduro lidera a copy — não só o tema (PARE — pedido do Sávio, 28/jul).** Extrair do top-8 vigente o que os campeões FAZEM (não sobre o que falam) e escrever como regras de copy na pauta. Referência 28/jul/26: 7 de 8 eram conversa comercial com objeção→resposta; capa = reframe disruptivo ou a objeção literal (nunca cena editorial construída); miolo = scripts palavra-por-palavra; metodologia nomeada sustenta, nunca lidera; o mesmo reframe postado 2x pagou 2x (objeção não satura, muda a objeção da vez). *Sintoma que esta regra corrige: posts tecnicamente limpos mas "não alinhados com os que performam" — o Sávio sente na hora.*
- [ ] **Checagem de queimadas NO NÍVEL DO TEMA** (`headlines-repertoire.json`, bloco `queimadas`): a família da frase/tese proposta está queimada? Adjacência conta (ex.: "vou ver com meu financeiro" = família do marido; "manda a tabela" = família do quanto-custa). **Queimada é contador, não muro (30/jul):** o resultado da checagem é uma DECLARAÇÃO na pauta ("3º uso, último há 6 dias") e uma decisão, não um veto automático — repetir o que performou é a política. Reprova só se os usos caem em posts adjacentes no feed ou se a formulação é copiada inteira. Ver o item equivalente no `checklist-producao.md` §6.
